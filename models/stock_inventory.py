# -*- coding: utf-8 -*-
# Copyright 2018 Aurium Technologies (<http://www.auriumtechnologies.com>)
...
class StockInventory(models.Model):
    ...
    filter = fields.Selection(_get_available_filters,
                              string='Selection Filter', required=True)
        ...
        for inventory in self:
            if not inventory.processed:
                raise exceptions.Warning(
                    _("Loaded lines must be processed at least one time for inventory : %s") % (inventory.name))
